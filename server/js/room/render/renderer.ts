...
import { HIDDEN_ALPHA, createPawnPass, createPawnProgram } from "./pawn-pass.ts";
...
import { fastBeat, healthOf, slowBeat } from "../model/health.ts";
import { CONDITION_COLORS, SELECT_COLOR, actorColor, auraColor } from "../model/color.ts";
...
export function mountRenderer(mount: HTMLElement, state: State, role: Role, table?: Table): Renderer | null {
	...
	let pawnProgram = createPawnProgram(gl);
	let pawnPass = createPawnPass(gl, pawnProgram);
	let ghostPass = createPawnPass(gl, pawnProgram);
	...
	function rebuild(): void {
		...
		pawnProgram = createPawnProgram(gl);
		pawnPass = createPawnPass(gl, pawnProgram);
		ghostPass = createPawnPass(gl, pawnProgram);
