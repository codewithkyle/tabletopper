...
func (s *Store) FromRequest(r *http.Request) (UserSession, error) {
	...
	return UserSession{
		...
		Prefs: prefs.New(prefs.Stored{
			Theme:      string(row.Theme),
			Timezone:   row.Timezone,
			DateFormat: string(row.DateFormat),
			TimeFormat: string(row.TimeFormat),
			FollowTurn: row.FollowTurn,
			ShowBlood:  row.ShowBlood,
			PingVolume: int(row.PingVolume),
			TurnAlert:  row.TurnAlert,
			TurnVolume: int(row.TurnVolume),
			TurnNotify: row.TurnNotify,
		}),
