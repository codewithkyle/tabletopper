...
func TestAuthorizeCoversEveryWireCommand(t *testing.T) {
	...
	tests := []struct {
		...
		{"note.set", &NoteSet{Layer: w.layer, Q: 0, R: 0, Title: "Tower"}, ok, ok, ok},
		{"note.reveal", &NoteReveal{Layer: w.layer, Q: 0, R: 0, Revealed: true}, ok, CodeForbidden, CodeForbidden},
		{"note.remove", &NoteRemove{Layer: w.layer, Q: 0, R: 0}, ok, CodeForbidden, CodeForbidden},
