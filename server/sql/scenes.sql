...
SELECT s.id, s.name, s.keep_changes, s.created_at, s.updated_at, a.id AS preview_id
FROM scenes s
LEFT JOIN assets a ON a.id = s.preview_id AND a.owner_id = s.owner_id AND a.type = 'map'
WHERE s.owner_id = ?
ORDER BY s.name;
-- name: GetScene :one
SELECT id, owner_id, name, body, keep_changes, preview_id, created_at, updated_at
FROM scenes
