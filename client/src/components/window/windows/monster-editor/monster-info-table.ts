...
class MonsterInfoTable extends SuperComponent<IMonsterInfoTable>{
    ...
    private updateDesc: EventListener = (e: Event) => {
        ...
        const index = parseInt(target.dataset.index);
        const updatedModal = this.get();
        updatedModal.rows[index].value = target.value;
        this.set(updatedModal, true);
    }
        ...
        const view = html`
            ...
            return html`
                    <table-row>
                        ...
                        <textarea name="${this.model.name}-value" required placeholder="Description" rows="4" @input=${this.updateDesc} data-index="${index}">${row.value}</textarea>
                    </table-row>
                `;
        })}
